  ...
  methods: {
    ...
    async getUserDetailById() {
      ...
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        // 如果以及存在图片 获取图片信息把它存入upload组件的fileList里显示
        // 注意：原先存在的图片并没有upload属性，需要人为添加
        // 这样 如果没有修改图片 点击更新才能够保存成功
    ...
    async saveUser() {
      ...
      // 点击保存调用接口前需要确认图片是否上传完毕 根据upload: true属性
      if (fileList.some(item => !item.upload)) {
        this.$message.warning('您当前还有图片没有上传完成！')
        return
      }
      // 通过合并 得到一个新对象 要把userInfo里的staffPhoto的url更新为新的图片的url
      await saveUserDetailById({ ...this.userInfo, staffPhoto: fileList && fileList.length ? fileList[0].url : ' ' }) // 接口原因要求没有设置图片的返回一个空字符串 且必须带一个空格
